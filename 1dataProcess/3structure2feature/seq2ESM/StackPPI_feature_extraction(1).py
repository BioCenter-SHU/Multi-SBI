...
def NMBroto(input_data, props=['CIDH920105', 'BHAR880101', 'CHAM820101', 'CHAM820102',
    ...
    if minSequenceLength(fastas) < nlag + 1:
        ...
        return 0
    fileAAidx = os.path.split(os.path.realpath(__file__))[0] + r'\data\AAidx.txt' if platform.system() == 'Windows' else \
    sys.path[0] + '/data/AAidx.txt'
...
def Geary(input_data, props=['CIDH920105', 'BHAR880101', 'CHAM820101', 'CHAM820102',
    ...
    if minSequenceLength(fastas) < nlag + 1:
        print('Error: all the sequence length should be larger than the nlag+1: ' + str(nlag + 1) + '\n\n')
        return 0
    AAidx_file = os.path.split(os.path.realpath(__file__))[
                     0] + r'\data\AAidx.txt' if platform.system() == 'Windows' else sys.path[0] + '/data/AAidx.txt'
    ...
    for i in fastas:
        ...
        for prop in range(len(props)):
            ...
            for n in range(1, nlag + 1):
                if len(sequence) > nlag:
                    rn = (N - 1) / (2 * (N - n)) * ((sum(
                        [(AAidx[prop][index.get(sequence[j], 0)] - AAidx[prop][index.get(sequence[j + n], 0)]) ** 2 for
...
def PAAC(input_data, lambdaValue=30, w=0.05):
    ...
    dataFile = os.path.split(os.path.realpath(__file__))[
                   0] + r'\data\PAAC.txt' if platform.system() == 'Windows' else re.sub('codes$', '', os.path.split(
        os.path.realpath(__file__))[0]) + '/data/PAAC.txt'
...
# 能用
    ...
    vector = CTDT(sqe_data)
    csv_data = pd.DataFrame(data=vector)
    csv_data.to_csv('CTDT_out.csv', header=False, index=False)
# 能用
elif (option == "3"):

    vector = CTDD(sqe_data)
    csv_data = pd.DataFrame(data=vector)
    csv_data.to_csv('CTDD_out.csv', header=False, index=False)
# 能用
elif (option == "4"):

    vector = NMBroto(sqe_data, nlag=0)
    csv_data = pd.DataFrame(data=vector)
    csv_data.to_csv('NMBroto_out.csv', header=False, index=False)
# 能用
elif (option == "5"):

    vector = Moran(sqe_data, nlag=10)
    csv_data = pd.DataFrame(data=vector)
    csv_data.to_csv('Moran_out.csv', header=False, index=False)
# 能用
elif (option == "6"):

    vector = Geary(sqe_data, nlag=10)
    csv_data = pd.DataFrame(data=vector)
    csv_data.to_csv('Geary_out.csv', header=False, index=False)
...
# 不能用
    ...
    vector = PAAC(sqe_data, lambdaValue=30, w=0.05)
    csv_data = pd.DataFrame(data=vector)
    csv_data.to_csv('PAAC_out.csv', header=False, index=False)
